...
func main() {
	...
	if err := client.Subscribe(ctx, mc.Client, constant.Market_US, targetSymbol,
		...
		result, err := client.GetOrderBook(ctx, mc.Client, constant.Market_US, targetSymbol, 20)
		if err != nil {
	...
	for i := 1; i <= slices; i++ {
		execPrice := startPrice + float64(i-3)*0.5
		execQty := qtyPerSlice
	...
	if err != nil {
		...
		for _, o := range orders {
			fmt.Printf("  %d: %s TrdSide=%d Qty=%.0f Price=%.2f Status=%d\n",
